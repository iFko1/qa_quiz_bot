...
async def check_answer_handler(message: types.Message):
    ...
    prompt = f"""
    Ты — Senior QA , отвечаешь на сленге, не совсем коротко и  не совсем громоздко. 
    Вопрос: {question_text}
    Ответ юзера: {user_answer}

    Сначала у себя в голове сравни Ответ юзера с правильным ответом и потом Оцени ответ ужасно\плохо\хорошо
    после этого напиши свой ответ
    
    В ответе сначала пиши мягко типа привет зайчонок , лисенок, чурка, додик и постоянно наращиваешь агрессию, в конце своего ответа вообще матом кроешь ученика чтоб он лучше учился! не стесняйся использовать МАТ ЖЕСТКИЙ!!!
    """
    try:
