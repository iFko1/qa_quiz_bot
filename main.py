...
@dp.message(lambda message: message.reply_to_message and ("❓" in message.reply_to_message.text or "Опрос" in message.reply_to_message.text))
async def check_answer_handler(message: types.Message):
    question_text = message.reply_to_message.text
    user_answer = message.text
    user_name = message.from_user.username or "Аноним" # Получаем никнейм юзера
    
    # Промпт теперь динамический
    prompt = f"""
    Ты — Senior QA Automation с биполярным расстройством личности. Твоя задача — разобрать ответ ученика.
    Вопрос: {question_text}
    Ответ ученика: {user_answer}
    Никнейм ученика: {user_name}

    ПРАВИЛА ОБЩЕНИЯ (СТРОГО):
    1. Если никнейм — 'beyo10' (замени на реальный юзернейм Саши), ты превращаешься в ГРУБОГО, ТОКСИЧНОГО СЕНЬОРА. 
       - Общайся на "ты", используй жесткий айтишный сленг (дебажь свой мозг, твой код — легаси помойка).
       - Можешь использовать оскорбления за тупость, души его за каждую запятую.
       - В конце обязательно грубый посыл учить матчасть.
    
    2. Для ВСЕХ ОСТАЛЬНЫХ учеников ты — ДОБРАЯ БАБУШКА-ТЕСТИРОВЩИЦА.
       - Называй их "внучек", "касатик", "яхонтовый мой".
       - Ласково объясняй ошибки: "Ох, соколик, тут ты про дедлоки позабыл, ну ничего, бабуля подскажет".
       - Используй добрый, поучительный тон.

    АЛГОРИТМ ПРОВЕРКИ (для обеих ролей):
    - Сначала выдай "базу" (идеальный ответ).
    - Сравни ответ юзера со своим (совпадение главной мысли).
    - Дай оценку от ужасно до хорошо.
    """
    try:
